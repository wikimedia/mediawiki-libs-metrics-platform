...
class ContextControllerTest extends TestCase {
	...
	public function testAddContextValues(): void {
		...
		$this->assertFalse( $pageData["is_redirect"] );
		...
		$this->assertFalse( $siteData["is_production"] );
		$this->assertTrue( $siteData["is_debug_mode"] );
		$this->assertSame( "zhwiki", $siteData['database'] );
		$this->assertSame( "zh", $siteData["site_content_language"] );
		$this->assertSame( "zh-tw", $siteData["site_content_language_variant"] );

		$this->assertTrue( $userData["is_logged_in"] );
		...
		$this->assertFalse( $userData["is_bot"] );
		$this->assertFalse( $userData["is_temp"] );
		$this->assertSame( "zh", $userData["language"] );
		$this->assertSame( "zh-tw", $userData["language_variant"] );
		$this->assertTrue( $userData["can_probably_edit_page"] );
